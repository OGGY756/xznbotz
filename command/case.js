...
//greetings
    ...
    try {
      groupMet = await haruka.groupMetadata(anu.jid);
      ...
      console.log(anu);
      try {
        pp_user = await haruka.getProfilePicture(mem);
      } catch (e) {
        pp_user =
          "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60";
      }
      try {
        pp_grup = await haruka.getProfilePicture(anu.jid);
      } catch (e) {
        pp_grup =
          "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60";
      }
      if (anu.action == "add" && mem.includes(haruka.user.jid)) {
        haruka.sendMessage(anu.jid, "Assalamualaikum", "conversation");
      }
...
if (anu.action == 'add' && !mem.includes(haruka.user.jid)) {
             ...
             const thu = await haruka.getStatus(anu.participants[0], MessageType.text)
             const num = anu.participants[0]
             const bosco1 = await haruka.prepareMessage("0@s.whatsapp.net", hehe, MessageType.location,{ thumbnail: hehe})
			 const bosco2 = bosco1.message["ephemeralMessage"] ? bosco1.message.ephemeralMessage : bosco1
                ...
                welcomeBut = [{buttonId:`#owner`,buttonText:{displayText:'OWNER'},type:1}, {buttonId:`bebantot`,buttonText:{displayText:'Welcome Beban'},type:1}]
            ...
            if (anu.action == 'remove' && !mem.includes(haruka.user.jid)) {
                const mdata = await haruka.groupMetadata(anu.jid)
                const num = anu.participants[0]
                const bosco3 = await haruka.prepareMessage("0@s.whatsapp.net", hehe, MessageType.location,{ thumbnail: hehe})
			    const bosco4 = bosco3.message["ephemeralMessage"] ? bosco3.message.ephemeralMessage : bosco3
                let w = haruka.contacts[num] || { notify: num.replace(/@.+/, '') }
                anu_user = w.vname || w.notify || num.split('@')[0]
                time_welc = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
                time_wel = moment.tz('Asia/Kolkata').format("hh:mm")
                memeg = mdata.participants.length
                out = `*Sayonara B* \n@${num.split('@')[0]}\n *Leaving From* : *${mdata.subject}*\n *Rs N P* \n${time_wel} -  ${time_welc}`
                goodbyeBut = [{buttonId:`#h`,buttonText:{displayText:'Good Bye Beban'}, type:1}]
		...
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			...
			ini_list.push({
				...
				hehe = await haruka.sendMessage(from, {
					...
					haruka.sendMessage(from, `Here My Contact For Owner And My Partner ü§ù`, text, {quoted: hehe})
				}
			break
case 'donasi':
var aprian = `
...
setTimeout( () => {
haruka.sendMessage(from, '_[  ] 10 SecondÖ_', text) // ur cods
}, 20000) // 1000 = 1s,
setTimeout( () => {
haruka.sendMessage(from, '_[  ] 20 Second_Ö', text) // ur cods
}, 10000) // 1000 = 1s,
setTimeout( () => {
haruka.sendMessage(from, '_[  ] 30 Second_Ö', text) // ur cods
}, 2500) // 1000 = 1s
...
Pertanyaan = randKey.result.soal
setTimeout( () => {
haruka.sendMessage(from, '* Answer :* \n```'+randKey.result.jawaban +'```', text, {quoted: fgif}) // ur cods
}, 30000) // 1000 = 1s,
setTimeout( () => {
reply('_Your Time Is Empty_')
}, 29000)
setTimeout( () => {
haruka.sendMessage(from, '_[ ] 10 SecondÖ_', text) // ur cods
}, 20000) // 1000 = 1s,
...
haruka.sendMessage(from, buffer, video, {mimetype: 'video/mp4', quoted: mek, caption: 'Nih kak'})
break
case 'tiktokwm':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
reply(lang.wait())
...
_*Wait A Minute Prosess Upload Media......*_`
reply(result)
